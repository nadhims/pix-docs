---
sidebar_position: 2
title: Feature Comparison
description: Side-by-side comparison of features available on Pix Starter, Day Pass, and Pix Pro plans.
tags: [pricing, features, comparison]
---

# Feature Comparison

Use the table below to see exactly what's included in each Pix plan. Day Pass and Pix Pro share the same feature set — the difference is billing (24-hour vs. monthly/yearly).

## Features by Plan

| Feature | Starter (Free) | Day Pass ($5/day) | Pix Pro ($40/mo or $300/yr) |
|---|:---:|:---:|:---:|
| **Booths covered** | 1 (free) | 1 per Day Pass | 1 per subscription — one licence per booth |
| **Templates** | Default only | Unlimited | Unlimited |
| **Template Editor** | -- | Yes | Yes |
| **UI Editor** | -- | Yes | Yes |
| **Photo Filters** | -- | Yes | Yes |
| **Soft Files** (Composite, GIF, Live Photo, Singles) | -- | Yes | Yes |
| **Printer Support** | Yes | Yes | Yes |
| **Microsites** (QR sharing + customer page) | -- | Yes | Yes |
| **Offline Operation** | Yes | Yes | Yes |
| **Automatic Cloud Sync** | -- | Yes | Yes |
| **Real-Time Analytics** | -- | Yes | Yes |
| **Camera Support** (Canon DSLR + Webcam) | Yes | Yes | Yes |
| **Booth Health Monitoring** | -- | Yes | Yes |
| **License Management** | -- | Yes | Yes |
| **Payment Gateway** | -- | Yes | Yes |
| **Custom Branding** | -- | Yes | Yes |

:::tip
Starter is print-only: customers get a physical print, no digital/QR sharing and no soft files. Paid plans (Day Pass, Pix Pro) unlock soft files, the Template & UI editors, microsites, analytics, and management features that make your booth business-ready.
:::

## What "Offline Operation" Means

All plans support capturing photos without an internet connection. The difference is what happens afterward:

- **Starter**: Prints directly, no cloud upload, no soft files, no microsites.
- **Day Pass / Pix Pro**: Photos queue locally and automatically upload when connectivity returns. Soft files and microsites become available once the upload completes.

## What "Payment Gateway" Means

Day Pass and Pix Pro let you connect a payment provider (Xendit, DOKU, or Midtrans) so customers can pay per photo session or per custom frame directly at the booth, with Pix taking a 0% cut. This is optional — you can run a booth without customer-facing payments if your business model doesn't require it.

:::info
Need help deciding? Every new account starts with a free 3-day trial of Pix Pro, so you can try everything before choosing. Running your kiosk full-time? Pix Pro Yearly offers the best value at $300/year.
:::
